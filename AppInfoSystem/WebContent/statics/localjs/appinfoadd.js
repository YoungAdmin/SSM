		...
		$.ajax({
			...
			success:function(data){//data：返回数据（json对象）
				if(data.APKName == "empty"){//参数APKName为空，错误提示
					$("#APKNameMessage").html("*APKName为不能为空！").css("color","red");
					$("#send").attr("type","button").css("backgound-color","#999");
				}else if(data.APKName == "exist"){//账号不可用，错误提示
					$("#APKNameMessage").html("*该APKName已存在，不能使用！").css("color","red");
					$("#send").attr("type","button").css("backgound-color","#999");
				}else if(data.APKName == "noexist"){//账号可用，正确提示
					$("#APKNameMessage").html("*该APKName可以使用！").css("color","green");
					$("#send").attr("type","submit").css("backgound-color","#999");
				}
			},
			error:function(data){//当访问时候，404，500 等非200的错误状态码
				$("#APKNameMessage").html("*请求错误！");
				$("#send").attr("type","button").css("backgound-color","#999");
			}
		});
	});
